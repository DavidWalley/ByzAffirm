...
ByzNode.prototype.MakeMemo = function(a_sData_base64) {
  ...
  if (1 < as.length) {
    ...
    if (0 <= me._a2sMemos[iMemosNode][iMemo].indexOf(ByzNode.sLINK)) {
...
ByzNode.prototype.sHowMuchIKnow = function() {
  var me = this;
  var r_s = me._iWhich + ByzNode.sFIELDsPLIT + me._whenNowMe_ms();
  var n = me._a2sMemos.length;
  for (var i = 0; i < n; i++) {
    r_s += ByzNode.sFIELDsPLIT + me._a2sMemos[i].length;
  }
  return r_s;
};
...
ByzNode.prototype._whenMade = function(a_iNode, a_iMemo) {
  ...
  for (var i = 0; i < me._nNodes; i++) {
    ...
    for (var j = 0; j < nSize; j++) {
      asFields = me._a2sMemos[i][j].split(ByzNode.sFIELDsPLIT);
      asData = asFields[3].split(ByzNode.sLINK);
      if (1 === asData.length && +asFields[0] === a_iNode && +asFields[1] === a_iMemo) {
        return +asFields[2];
      }
    }
  }
...
ByzNode.prototype.sGetNewsForCaller = function(a_sHowMuchTheyKnow) {
  ...
  var as = a_sHowMuchTheyKnow.split(ByzNode.sFIELDsPLIT);
  if (as.length < 3) {
    me._Tell("Error1: " + me._sName + " " + JSON.stringify(as));
    return "Error1";
  }
  var iCaller = +as[0];
  var whenCaller_ms = +as[1];
  var tookDelta_ms = me._whenNowMe_ms() - whenCaller_ms;
  if (tookDelta_ms < -ByzNode.takeOKsHORT_ms) {
    me._Tell("TIMING Error2 '" + a_sHowMuchTheyKnow + "' " + whenCaller_ms + " (" + tookDelta_ms + ").");
    return "Error2";
  }
  if (ByzNode.takeOKsHORT_ms < tookDelta_ms) {
    me._Tell("TIMING Error3 '" + a_sHowMuchTheyKnow + "' " + whenCaller_ms + " (" + tookDelta_ms + ").");
    return "Error3";
  }
  ...
  var iIAmAt = 0;
  var iOtherNode = 0;
  for (var i = 2; i < as.length; i++, iOtherNode++) {
    iOtherNodeAt = parseInt(as[i], 10);
    iIAmAt = me._a2sMemos[iOtherNode].length;
    for (var j = iOtherNodeAt; j < iIAmAt; j++) {
      r_s += " " + ByzNode.sMEMOsPLIT + " " + me._a2sMemos[iOtherNode][j];
    }
  }
...
ByzNode.prototype.Hark = function(a_sDebugNotes, a_sAllMemos) {
  var me = this;
  var asMemos = a_sAllMemos.split(ByzNode.sMEMOsPLIT);
  var nMemos = asMemos.length;
  if (nMemos <= 1) {
    return;
  }
  me._Tell("Hark: >>" + a_sDebugNotes + "<<. [[" + G.sSHRINK(a_sAllMemos) + "]]");
  var as = [];
  var anLengthWas = [];
  var i = 0;
  for (; i < me._nNodes; i++) {
...
ByzNode.prototype._Hark_Process = function(a_sMemo) {
  ...
  var as = a_sMemo.split(ByzNode.sFIELDsPLIT);
  if (as.length < 5) {
    me._Tell("???16");
    return;
  }
  ...
  as = sPlain.split(ByzNode.sFIELDsPLIT);
...
ByzNode.prototype._Cleanup = function(a_anLengthWas) {
  ...
  for (var i = 0; i < me._nNodes; i++) {
    ...
    for (var j = a_anLengthWas[i]; j < n; j++) {
      ...
      var as = s.split(ByzNode.sFIELDsPLIT);
